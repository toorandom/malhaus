#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# MCP — analyze a remote file by URL
#
# URL submission is identical for REST and MCP — no binary transfer needed.
# The MCP server passes the URL to /analyze as a JSON parameter.
# ---------------------------------------------------------------------------

HOST="${MALTRIAGE_HOST:-https://your-domain.com}"
TOKEN="${MALTRIAGE_TOKEN:?Set MALTRIAGE_TOKEN to your mh_... API key}"
URL="${1:?Usage: $0 <https://example.com/file.exe>}"
POLL_INTERVAL=5
