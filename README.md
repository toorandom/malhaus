...
- **Compression curve** — the file is compressed at every level with zlib, bz2, and lzma. The resulting ratio curves distinguish truly random data (flat near 1.0) from structured or already-compressed content. Grounded in [Kolmogorov complexity](https://en.wikipedia.org/wiki/Kolmogorov_complexity) and [Li et al. — normalized compression distance](https://arxiv.org/abs/cs/0312044).
...
- **Byte-trigram point cloud** — every overlapping triple of bytes `(b[i], b[i+1], b[i+2])` is treated as a 3D coordinate — an original application of delay-coordinate embedding to malware byte streams, inspired by [Floris Takens](https://en.wikipedia.org/wiki/Floris_Takens) and [Takens' theorem](https://en.wikipedia.org/wiki/Takens%27s_theorem). PCA reduces the cloud to 2D and HDBSCAN groups points into density clusters, each rendered in a distinct colour. See also [Nataraj et al. (2011), VizSEC — malware visualization via raw byte sequences](https://dl.acm.org/doi/10.1145/2016904.2016908).
